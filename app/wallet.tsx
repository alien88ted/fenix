      ...
      <main className="max-w-sm mx-auto px-6 py-8" role="main">
        ...
        {currentView === "send" && (
          <SendView
            ...
            handleSendContinue={handleSendContinue}
...
function SendView({ sendStep, sendData, errors, balance, walletAddress, setSendData, handleSendContinue, handleSendConfirm, setSendStep }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {sendStep === 2 && <SendConfirmStep sendData={sendData} handleSendConfirm={handleSendConfirm} setSendStep={setSendStep} />}
      {sendStep === 3 && <SendProcessingStep />}
      {sendStep === 4 && <SendSuccessStep sendData={sendData} />}
    </div>
  )
}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {["10", "100", balance.replace(',', '')].map((amount, idx) => (
                <Button
                  key={idx}
                  onClick={() => setSendData({...sendData, amount})}
      ...
      <CardContent className="p-6">
        <div className="space-y-6">
          ...
          <div className="space-y-4 p-4 bg-secondary/20 rounded-lg border border-border/30">
            {[
              ["To", sendData.address, "font-mono text-right max-w-32 truncate"],
              ["Amount", `$${sendData.amount} USDT`, "font-semibold"],
              ["Network Fee", "$0.50", ""],
              ...(sendData.memo ? [["Memo", sendData.memo, ""]] : [])
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <div className="p-3 bg-secondary/20 rounded-lg border border-border/30">
                  <p className="text-xs font-mono text-foreground break-all select-all">{address}</p>
                </div>
        ...
        <CardContent className="p-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button className="text-xs text-primary hover:text-primary/80 p-0 h-auto bg-transparent hover:bg-transparent" variant="ghost">
                View All
              </Button>
            </div>
            ...
            <div className="space-y-3">
              {[
                { amount: "250.00", time: "2 hours ago", from: "0x1234...5678" },
                { amount: "100.00", time: "1 day ago", from: "0xabcd...efgh" },
                { amount: "75.50", time: "3 days ago", from: "0x9876...5432" }
              ].map((tx, index) => (
